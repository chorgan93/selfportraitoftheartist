...
public class SacramentCombatOptionUndertextS : MonoBehaviour {

	private SacramentCombatActionOptionS myOption;
	...
	public void ActivateUndertext(SacramentCombatActionOptionS myO){
		Initialize(myO);
		shakeCountdown = Random.Range(0, shakeRate);
		fadeCol = myText.color;
		fadeCol.a = 0f;
	...
	private void Initialize(SacramentCombatActionOptionS myOpt){
		if (!_initialized){
		myOption = myOpt;
